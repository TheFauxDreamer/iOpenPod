...
def _detect_border(image_rgb, threshold: int = 8) -> tuple:
    """Detect and crop a solid-color border/frame around artwork.

    Returns the cropped image (or original if no border detected).
    ...
    # Check if left edge is all roughly the same color
    same_count = 0
